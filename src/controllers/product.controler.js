...
function calculateDiscountedPrice(pricing, promotions) {
    ...
    return pricing.base_price - (pricing.base_price * promotions.discount_percentage / 100);
}
...
export const createProduct = asyncHandler(async (req, res) => {
    ...
    pricing.forEach(price => {
        if (!price.base_price || !price.cost) {
            throw new ApiError(400, `Missing required pricing fields with size ${price.size}`);
        }
...
export const createMultipleProducts = asyncHandler(async (req, res) => {
    ...
    const productsToInsert = products.map(productData => {
        const { name, description, category_id, subcategory_id, pricing, inventory, images, attributes, promotions } = productData;
    ...
    const newProducts = await Product.find({ _id: { $in: insertedProducts.map(p => p._id) } })
        .select('-isActive -pricing.cost -attributes.weight -promotions.sale_end_date');

    res.status(201).json(new ApiResponse(201, newProducts, 'Products created successfully'));
});
...
export const searchProducts = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(new ApiResponse(200, products, `${products.length} Search Result Found`));
});
